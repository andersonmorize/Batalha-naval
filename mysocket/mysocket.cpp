...
MYSOCKET_STATUS mysocket::init()
{
  ...
  return(iResult==0 ? MYSOCKET_OK : MYSOCKET_ERROR);
}
...
MYSOCKET_STATUS tcp_mysocket::connect(const char *name, const char *port)
{
  ...
  intResult = getaddrinfo(name, port, &hints, &result);
  if (intResult != 0) {
      return MYSOCKET_ERROR;
  }
...
MYSOCKET_STATUS tcp_mysocket::read(char *dado, size_t len, long milisec) const
{
  ...
  if (milisec>=0)
  {
    ...
    if (iResult==MYSOCKET_ERROR || iResult==MYSOCKET_TIMEOUT)
    {
      return iResult;
    }
  }
...
{
  return read((char*)&num,sizeof(num),milisec); // sizeof(num) == 4
}
...
{
  return write((char*)&num,sizeof(num)); // sizeof(num) == 4
}
...
{
  static char buffer[MYSOCKET_TAM_MAX_STRING+1];
  int32_t len;
  MYSOCKET_STATUS iResult;

  // Leh o numero de caracteres da string
  iResult = read_int(len,milisec);
  if (iResult==MYSOCKET_ERROR || iResult==MYSOCKET_TIMEOUT || iResult==MYSOCKET_DISCONNECTED)
  {
    // Erro ou timeout ou desconexao
    msg = "";
    return iResult;
  }
  if (len > MYSOCKET_TAM_MAX_STRING)
  {
    // Msg muito grande
    msg = "";
    return MYSOCKET_ERROR;
...
MYSOCKET_STATUS tcp_mysocket_server::listen(const char *port, int nconex)
{
    ...
    intResult = getaddrinfo(NULL, port, &hints, &result);
    if (intResult != 0) {
      return MYSOCKET_ERROR;
    }
...
MYSOCKET_STATUS mysocket_queue::wait_read(long milisec)
{
  ...
  if (intResult<0) return MYSOCKET_ERROR;
  if (intResult==0) return MYSOCKET_TIMEOUT;
  return MYSOCKET_OK;
}
